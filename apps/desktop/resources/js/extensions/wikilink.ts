...
export function wikiLinkSuggestion(): Omit<SuggestionOptions<SuggestionItem>, 'editor'> {
    return {
        char: '[[',
        items: async ({ query }) => {
            ...
            if (query.length > 0) {
                ...
                const pages = data.filter((n: { parent_id: string | null }) => !n.parent_id);
                for (const page of pages.slice(0, 10)) {
                    items.push({ id: page.id, content: page.content || '[untitled]' });
                }
                ...
                if (!exactMatch) {
                    items.unshift({ id: `create:${query}`, content: query, isCreate: true });
                }
            } else {
